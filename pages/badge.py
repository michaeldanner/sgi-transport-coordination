# pages/saturday.py
from dash import dcc, html, dash_table
from pages import utils
import dash_bootstrap_components as dbc


def get_layout(df):
    df = df[0]
    layout = html.Div([
        dbc.Row([
            dbc.Col([
                html.Div([
                    dbc.Row(dbc.Col(html.H3(f"Departure Saturday, 11.05.2024 ({df.shape[0]})"), width={"size": 6, "offset": 1})),
                    dbc.Row([
                        dbc.Col(dash_table.DataTable(
                            id='table',
                            columns=utils.col_depart,
                            data=df.to_dict('records'),
                            # filter_action="native",  # Enable filtering
                            markdown_options={"html": True},  # Allow HTML content
                            style_data_conditional=[
                                {'if': {'state': 'selected'}, 'backgroundColor': 'rgba(0, 116, 217, 0.3)',
                                 'border': '1px solid blue'}
                            ],
                            css=[{"selector": "P", "rule": "margin-top: 0; margin-bottom: 0"}],
                            fill_width=False,
                        ), width=2)
                    ])
                ])
            ]),
            dbc.Col(html.Div(id='row-info', style={'margin-top': '0px'}), width=4)
        ])  # end Main Row
    ])

    layout = html.Div([
        dbc.Container([
            dbc.Row([
                dbc.Col([
                    dbc.Card([
                        dbc.CardImg(src=(row['Flag_url']), top=True, style={'width': '100%'}),
                        dbc.CardBody([
                            html.H5(f"{row['First Name']} {row['Surname']}", className="card-title"),
                            html.P(row['Country'], className="card-text"),
                            html.P(row['Flag'])
                        ])
                    ], style={'width': '3.5in', 'height': '5.7in'})  # Approx 9x14.5 cm
                ]) for index, row in df.iterrows()
            ], className='g-4')
        ], fluid=True)
    ])
    return layout
